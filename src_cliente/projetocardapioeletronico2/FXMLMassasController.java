 ...
 * @author isabe
 */
public class FXMLMassasController implements Initializable {
    
    
    @FXML
    private Button btnMenuCarrinho;
    @FXML
    private Pane paneMassas;
    int a = 15;
    ...
    public void initialize(URL url, ResourceBundle rb) {
        try {
            criarCategoriaMassas() ;
        } catch (IOException ex) {
            Logger.getLogger(FXMLMassasController.class.getName()).log(Level.SEVERE, null, ex);
        }
    }   
    ...
    public void criarCategoriaMassas() throws IOException {
        ...
        for (Produto prd : daoproduto.getbuscarProdutosBD()) {
            ...
            if (prd.getCategoria().equalsIgnoreCase(categoria)) {
                try {
                    ...
                    paneMassas.getChildren().add(telinhaProduto);
